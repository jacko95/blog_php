<?php
    ...
    // $sql = $db->query("SELECT * FROM post_likes WHERE post_id = '".$post_id."'");
    ...
    if($count == 1){
        // $sql = $db->query("SELECT * FROM post_likes WHERE post_id = '".$post_id."'");
        // $data = mysqli_fetch_array($sql);
        $sum = $data['total_like'] - 1;//problema con data[]
        $db->query("UPDATE post_table SET total_like = '".$sum."' WHERE id = '".$post_id."'");
        $db->query("DELETE FROM post_likes WHERE post_id = '".$post_id."'");
        ...
        $db->query("UPDATE post_likes SET total_like = '".$sum."' WHERE id = '".$post_id."'");
    }
